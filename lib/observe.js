...
export async function observeServer({ cmd, args, env = {}, protocolVersions = PROTOCOL_VERSIONS, spawnTimeout = 45_000, cwd } = {}) {
  const client = new McpStdioClient({ cmd, args, env, spawnTimeout, cwd });
  const observation = {
    cmd: [cmd, ...args].join(" "),
    serverInfo: null,
