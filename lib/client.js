...
export class McpStdioClient {
  constructor({ cmd, args = [], env = {}, spawnTimeout = 60_000, cwd, detached = false, inheritEnv = true } = {}) {
    this.cmd = cmd;
    this.args = args;
    this.env = env;
  ...
  async start() {
    return new Promise((resolve, reject) => {
      const baseEnv = this.inheritEnv ? process.env : { PATH: process.env.PATH ?? "" };
  ...
  async stop() {
    ...
    // When detached, the child is its own process group leader; kill the whole
    // group for best-effort process-tree cleanup. Otherwise kill the main process.
    if (this.detached) {
