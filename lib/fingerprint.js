...
export async function fingerprint(spec, { env = {}, manifestPath = null, pinStore = null, protocolVersions, localCmd = null } = {}) {
  const card = {
    spec,
    at: new Date().toISOString(),
    package: localCmd ? { ok: false, error: "local command — package identity check skipped" } : await packageIdentity(spec),
    observation: null,
    manifest: null,
    provenance: null,
    binding: null,
  ...
  if (localCmd) {
    card.observation = await observeServer({ cmd: localCmd.cmd, args: localCmd.args, env: { ...env, ...localCmd.env }, cwd: localCmd.cwd, protocolVersions });
  } else {
